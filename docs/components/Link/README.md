...
Avoid using the [external icon](https://polaris.shopify.com/icons?icon=ExternalIcon&q=external), as it can add unnecessary visual load inside a sentence or when accompanied by other content. Instead, add clarity to external links through clear link text and predictable placement of the link in a merchant’s workflow.
