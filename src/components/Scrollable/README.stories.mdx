...
import { Scrollable, ScrollTo } from '@/polaris-vue';
...
<Meta
  ...
  argTypes={{
    ...
    ScrollTo: {
      description: 'Wrap it around content that you want to scroll to',
      control: { disable: true },
      table: {
        category: 'ScrollTo Component',
        type: {
          summary: '<ScrollTo> Content </ScrollTo>',
        },
      },
    },
  }}
...
such as terms of service, that allows for scrolling so merchants can expose more text as they read.

<br/><br/>

### Default scrollable container

Use when you need to make a region within the page independently scrollable.
It's often used in modals and other panes where it's helpful to provide an extra visual cue that content exists below or above the fold.

export const DefaultScrollable = (args) => ({
...
<Canvas>
  <Story story={DefaultScrollable} name="Default scrollable container"/>
</Canvas>
...
export const ScrollToExample = (args) => ({
  components: { Scrollable, ScrollTo },
  template: `
...
<Canvas>
  <Story story={ScrollToExample} name="Scroll to child component"/>
</Canvas>
...
export const Example = (args) => ({
  ...
  setup() {
    ...
    return {args, scrollBottom};
  },
  template: `
  <Scrollable :style="{height: '100px'}" v-bind="args" @scrolled-to-bottom="scrollBottom">
    <p>
      You must be 18 years or older or at least the age of majority in the
      jurisdiction where you reside or from which you use this Service.
    </p>
...
<Canvas style={{ backgroundColor: '#fff'}}>
  <Story story={Example} name="Example" height="150px"/>
</Canvas>
