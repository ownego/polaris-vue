...
export const PressedButton = (args, { argTypes }) => ({
  ...
  template: `<ButtonGroup :segmented="true">
    <ButtonGroupItem>
      <Button
        on-click="handleFirstButtonActive"
    ...
    <ButtonGroupItem>
      <Button
        on-click="handleSecondButtonActive"
  ...
  docs: {
    ...
    transformSource: (source) => {
      const tmpSource = source
        ...
        .replace(/\s*<\/template>/, '')
        .replace(/on-/g, 'v-on:')
        .replace(/:pressed="false"/, ':pressed="!isFirstButtonActive"')
        .replace(/pressed/, ':pressed="isFirstButtonActive"');
      const fullSource = dedent`
        data() {
          return {
            isFirstButtonActive: true,
